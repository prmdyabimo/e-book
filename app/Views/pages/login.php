  ...
  <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
    ...
    <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
      <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
        ...
        <form class="space-y-4 md:space-y-6" action="<?= base_url(); ?>/login/auth" enctype="multipart/form-data" method="POST">

          <?php if (session()->getFlashdata('msg-failed')) : ?>
            <div class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg dark:bg-red-200 dark:text-red-800" role="alert">
              <span class="font-medium"><?= session()->getFlashdata('msg-failed'); ?></span>
            </div>
          <?php endif; ?>

          <?php if (session()->getFlashdata('msg-success')) : ?>
            <div class="p-4 mb-4 text-sm text-green-700 bg-green-100 rounded-lg dark:bg-green-200 dark:text-green-800" role="alert">
              <span class="font-medium"><?= session()->getFlashdata('msg-success'); ?></span>
            </div>
          <?php endif; ?>
          ...
          <p class="text-sm font-light text-gray-500 dark:text-gray-400">
            No have an account? <a href="<?= base_url(); ?>/register" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Register here</a>
          </p>
        </form>
      </div>
    </div>
